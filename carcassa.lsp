...
(defun c:rimuovicassa ()
  ...
  (setq gr nil)
  (initget "Si No")
  (setq snrim (getkword (strcat "\nRimuovere pannelli da cassa? [Si/No]<Si>:")))
  (if (= snrim "Si")
    (progn
      (setq gr (ssget (list '(0 . "INSERT") '(8 . "tmp_selezione"))))
      (if gr
	(progn
	  (setq nr -1)
	  (while (setq ent (ssname gr (setq nr (+ 1 nr))))
	...
	(alert "Nessun Pannello associato a casse")
	)
      (setq ent nil)
      (setq hande nil)
      (setq codpas nil)
