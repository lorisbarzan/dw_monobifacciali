...
;cesoaiato retro
    ...
    ;rinfogli
     (setq rig_f (list (strcat "Retro cesoiato " ) (nth 14 svilcorr) (atof (rtos lunglam_r 2 1)) (atof (rtos halt_f_R 2 1)) (nth 16 svilcorr)
  ...
  ;sviluppo retro
    ...
    (progn
     ...
     (if (= "Si" (leggi ent "BLOCCOSVI"))
         ...
         (setq lrig (list (strcat "Sviluppo Retro " ) (atof (rtos lunglam_r 2 1)) (nth 16 svilcorr)
		      (atof (rtos halt_F_R 2 1)) (nth 14 svilcorr) forisn valsv (atoi (rtos (length listap) 2 0)) MODOP_ "Nr"))
       )
...
;;;	   )
	...
	(if (and (>= (nth 4 f) (nth 4 LUNG_M+H+A)) (<= (nth 4 f) (+ (nth 0 LUNG_M+H+A)  (nth 4 LUNG_M+H+A)))
		 (>= (nth 5 f) (nth 2 LUNG_M+H+A)) (<= (nth 5 f) (+ (nth 2 LUNG_M+H+A)  (nth 3 LUNG_M+H+A)))
		 )
	  (progn
	    (if (= 0 (nth 2 f))
...
;;;			    "\t" (RTOS (NTH 3 rig_m) 2 1) "\t" (RTOS (NTH 4 rig_m) 2 1) "\t" (RTOS (NTH 7 rig_m) 2 1)))
    ...
    (foreach m tabcert
      (if (and (= resisor (nth 0 tabcer)) (= pconf (nth 1 tabcert)) (= forisnlana (nth 2 tabcert)) (= fori (nth 3 tabcert)) (setq desctec (nth 4 tabcert)))
	)
...
;;;      )
    ...
    ;Cerco Pannelli STD MTD con DIVIDISX

(defun c:PROVAGIUN
    (if (and
	  (or (= modop "STD") (= modop "mtd"))
	  (= resis "B15"))
      (progn
	(setq prgiun nil)
	(setq piniz (list (- 10 (car (nth 0 listapt))) (- 10 (cadr (nth 0 listapt)))))
	(setq pfine (list (+ 10 (car (nth 0 listapt))) (+ 10 (cadr (nth 0 listapt)))))
	(if (ssget "X" piniz pfine '(0 . INSERT) '(2 . DIVIDI?X))
	  (Setq prgiun t)
	  )
	)
      )
    ...
    ;Cerco Pannelli STD MTD SPC MPC con DIVIDISX DIVIDIDX
    ...
    (if (and (= resis "B15"))
      (progn
	(if (and (/= "" (nth 19 svilcorr))(/= nil (nth 19 svilcorr)))
		(progn
		    (setq profilo (arcprof (nth 19 svilcorr) "Grezzo" (nth 20 svilcorr) 52))
;;;		      (setq hp1 (+ (/ (nth 20 svilcorr) 2) (/ (- halt (* (nth 20 svilcorr) 2)) 3)))
		  (setq hp1 (/ halt 2))
;;;		      (setq hp2 (+ (nth 20 svilcorr) (/ (- halt (* (nth 20 svilcorr) 2)) 3) (/ (- halt (* (nth 20 svilcorr) 2)) 3) (/ (nth 20 svilcorr) 2)))
		  (if (= 1 (nth 21 svilcorr))
		    (progn
...
;;;		      (setq LISTAPs (cons (list profilo 1 (- (+ lung lung2) 12) hp2 0 0) listaps))
	...
	(if (and (/= "" (nth 22 svilcorr))(/= nil (nth 22 svilcorr)))
		(progn
		    (setq profilo (arcprof (nth 22 svilcorr) "Grezzo" (- halt 60) 52))
		    (setq hp1 (/ halt 2))
		  (if (= 1 (nth 21 svilcorr))
		    (progn
